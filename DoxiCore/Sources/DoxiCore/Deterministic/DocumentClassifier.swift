...
public enum DocumentClassifier {
    ...
    static let rules: [Rule] = [
        ...
        Rule(type: .invoice, pattern: Pattern(#"tax\s+invoice|invoice\s+(?:no|number|#|date)|\bbill(?:ed)?\s+to\b|\binvoice\b"#), weight: 3),
        ...
        Rule(type: .businessLetter, pattern: Pattern(#"dear\s+(?:sir|madam|mr|ms)|yours\s+(?:faithfully|sincerely|truly)|^\s*subject\s*:"#, anchorsMatchLines: true), weight: 2),
    ...
    public static func classify(_ text: String) -> Result? {
        ...
        var best: (score: Double, rule: Rule, evidence: NSRange, inTitle: Bool)?
        for rule in rules {
            ...
            if best == nil || score > best!.score {
                best = (score, rule, titleHit ?? first.range, titleHit != nil)
            }
        }
